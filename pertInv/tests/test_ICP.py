...
class TestICP(TestCase):
    ...
    def test_simple(self):
        ...
        S_hat, q_values = pertInv.invariant_causal_prediction(X=np.column_stack((A,B)), y=C, z=E)
    ...
    def test_very_simple_R(self):
        ...
        _, q_values = pertInv.invariant_causal_prediction(X, y, z=E)
    ...
    def test_size(self):
        ...
        def one():
            ...
            B =  A + np.random.normal(scale=noise, size=[n]) / 3 + np.equal(E, 2);
            C =  B + np.random.normal(scale=noise, size=[n]);
            S_hat, q_values = pertInv.invariant_causal_prediction(X=np.column_stack((A,B)), y=C, z=E, alpha=alpha);
            return 0 in S_hat
